...
class UnifiedCard(Card):
    ...
    def __post_init__(self):
        ...
        if self.swipeableLayoutSlides and not all(
                s.mediumComponentKey in self.componentObjects and s.componentKey in self.componentObjects for s in
                self.swipeableLayoutSlides):
            raise ValueError('missing components')
        if any(c.destinationKey not in self.destinations for c in self.componentObjects.values() if
               hasattr(c, 'destinationKey')):
            raise ValueError('missing destinations')
        if any(b.destinationKey not in self.destinations for c in self.componentObjects.values() if
        ...
        mediaKeys.extend(a.iconMediumKey for l in (self.apps.values() if self.apps is not None else []) for a in l if
...
class _TwitterAPIScraper(snscrape.base.Scraper):
    ...
    def _get_api_data(self, endpoint, apiType, params):
        ...
        if apiType is _TwitterAPIType.GRAPHQL:
            params = urllib.parse.urlencode({'variables': json.dumps(params, separators=(',', ':'))},
        ...
        r = httpx.get(url=endpoint, params=params, headers=self._apiHeaders, proxies=proxies['https'])
    ...
    def _make_card(self, card, apiType, tweetId):
        ...
        if cardName in ('summary', 'summary_large_image', 'app', 'direct_store_link_app'):
            ...
            kwargs = _kwargs_from_map({'end_datetime_utc': 'endDate', 'last_updated_datetime_utc': 'lastUpdateDate',
                                       'duration_minutes': 'duration', 'counts_are_final': 'finalResults'})
            ...
            return PlayerCard(**_kwargs_from_map(
                {'title': 'title', 'description': 'description', 'card_url': 'url', 'player_image_original': 'imageUrl',
                 'site': 'siteUser'}))
        elif cardName in ('promo_image_convo', 'promo_video_convo'):
            kwargs = _kwargs_from_map({'thank_you_text': 'thankYouText', 'thank_you_url': 'thankYouUrl',
            ...
            if cardName == '745291183405076480:broadcast':
                keyKwargMap = {**keyKwargMap, 'broadcast_id': 'id', 'broadcast_url': 'url', 'broadcast_title': 'title',
                               'broadcast_thumbnail_original': 'thumbnailUrl'}
            else:
                keyKwargMap = {**keyKwargMap, 'id': 'id', 'url': 'url', 'title': 'title', 'description': 'description',
            ...
            kwargs = _kwargs_from_map({'event_id': 'id', 'event_title': 'title', 'event_category': 'category',
            ...
            kwargs = _kwargs_from_map({'newsletter_title': 'title', 'newsletter_description': 'description',
            ...
            return AmplifyCard(
                ...
                video=Video(
                    ...
                    variants=[VideoVariant(contentType=bindingValues['player_stream_content_type'],
                                           url=bindingValues['amplify_url_vmap'], bitrate=None)],
                ),
            )
        elif cardName == 'appplayer':
            kwargs = _kwargs_from_map(
                {'title': 'title', 'app_category': 'appCategory', 'player_owner_id': 'playerOwnerId',
                 'site': 'siteUser'})
            ...
            if bindingValues['cta'] not in ctas:
                ...
                return
            return MessageMeCard(**_kwargs_from_map({'recipient': 'recipient', 'card_url': 'url'}),
...
class TwitterTweetScraper(_TwitterAPIScraper):
    ...
    def get_items(self):
        ...
        if self._mode is TwitterTweetScraperMode.SINGLE:
            ...
            for obj in self._iter_api_data(url, _TwitterAPIType.GRAPHQL, variables, paginationVariables,
                                           direction=_ScrollDirection.BOTH):
                if not obj['data']:
                    continue
                yield from self._graphql_timeline_instructions_to_tweets(
                    obj['data']['threaded_conversation_with_injections']['instructions'],
            ...
            while queue:
                ...
                for obj in self._iter_api_data(url, _TwitterAPIType.GRAPHQL, thisVariables, thisPagVariables,
                                               direction=_ScrollDirection.BOTH):
                    if not obj['data']:
                        continue
...
if __name__ == '__main__':
    scraper = TwitterSearchScraper('(from:financialjuice) until:2023-01-11 since:2023-01-10', proxies={
        'http': '177.234.238.36:8080',
        'https': '177.234.238.36:8080'
    }, retries=0,
                                   top=True)
